...
export async function scrapeGrades(
  username?: string,
  password?: string
): Promise<any> {
...
async function extractGradeTableData(page: Page) {
  ...
  for (const row of rows) {
    ...
    const isSuccess = await row.evaluate((el) =>
      ...
      if (courseData) {
        ...
        if (assessmentButton) {
          try {
            ...
            await assessmentButton.evaluate((btn) =>
              btn.scrollIntoView({ behavior: "instant", block: "center" })
            );
            await assessmentButton.evaluate((b) => (b as HTMLElement).click());
            ...
            assessments = await page.evaluate(() => {
              ...
              const tableRows = document.querySelectorAll(
                "#AssessmentEntryGrid table tbody tr"
              );
            ...
            const closeButton = await page.$(
              "#myModal .modal-footer button[data-dismiss='modal']"
            );
            if (closeButton) {
            ...
            console.error(
              `Could not process assessment for "${courseData.courseTitle}". Error: ${e.message}`
            );
          }
        }
